        ...
        <div className="article-shell">
          <article className="article-body">
            ...
            {article.sections.map((section) => (
              <section key={section.heading} id={sectionId(section.heading)} className="reveal">
                <h2>{section.heading}</h2>
                {section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
                {section.bullets && <ul>{section.bullets.map((item) => <li key={item}>{item}</li>)}</ul>}
                {section.note && <aside className="article-field-note reveal"><strong>From the workroom</strong><p>{section.note}</p></aside>}
              </section>
            ))}
            <figure className="article-portfolio-proof reveal">
